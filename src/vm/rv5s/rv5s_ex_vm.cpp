...
RV5SEXVM::RV5SEXVM(bool silent) : VmBase(silent) {
    Reset();
}

RV5SEXVM::~RV5SEXVM() = default;

void RV5SEXVM::enableForwarding(bool enable) {
    forwarding_enabled_ = enable;
}

void RV5SEXVM::setBranchPredictorType(BranchPredictorType type) {
    switch(type) 
    {
 ...
 */
void RV5SEXVM::DumpState(const std::filesystem::path &filename) {
...
uint64_t RV5SEXVM::getWriteBackData() {
    ControlSignals control = mem_wb_reg_.control;
    uint64_t write_data = 0;
    switch (control.wb_src) {
        case WriteBackSrc::WB_FROM_ALU:
