/**
 * @file rvss_stall_vm.cpp
 * @brief Implementation of the 5-stage pipelined RISC-V VM with Hazard Detection and Resolution Using Stalls.
 */
...
RV5SStallVM::RV5SStallVM(bool silent) : VmBase(silent) {
    Reset();
}

RV5SStallVM::~RV5SStallVM() = default;

void RV5SStallVM::Reset() {
    program_counter_ = 0;
...
void RV5SStallVM::Decode_Stage() {

    if (flush_pipeline_) {          // set to true in case of a branch not taken instruction
        next_id_ex_reg_ = CreateBubble<ID_EX_Reg>();
        return;
    }
    ...
    // see if stall reqd 
    bool dataHazard = hazard_unit_.detectDataHazard(control, next_id_ex_reg_.rs1_index, next_id_ex_reg_.rs2_index, id_ex_reg_, ex_mem_reg_);
    if(dataHazard) 
    {
        stall_request_ = true;   // to indicate to the step function
        next_id_ex_reg_ = CreateBubble<ID_EX_Reg>();        // insert bubble
        return;
    }

}
void RV5SStallVM::Execute_Stage() {
    ...
    if(control.branch_op == BranchOp::JAL || control.branch_op == BranchOp::JALR) {
        ...
        switch (control.branch_op) {
            case BranchOp::BEQ:
                branch_taken = (execution_result == 0);
                break;
            case BranchOp::BNE:
                branch_taken = (execution_result != 0);
                break;
            case BranchOp::BLT:
                branch_taken = (execution_result == 1);
                break;
            case BranchOp::BGE:
                branch_taken = (execution_result == 0);
                break;
...
void RV5SStallVM::WriteBack_Stage() {
 ...
 */
void RV5SStallVM::DumpState(const std::filesystem::path &filename) {
