...
HAZARD_DIR = os.path.join(BASE_DIR, "2_instr")
...
CONSUMERS = ["add", "addi", "lw", "sw"]         # jalr tested separately later
        ...
        for consumer_instr in CONSUMERS:
            ...
            for p_rd in DEP_REGS:        
                for c_rs_other in OTHER_REGS: 
                    ...
                    if c_line:
                        test_lines = [p_line, c_line]
                        if producer_instr == "jal": 
                            test_lines = [p_line, f"addi {JUMP_TEST_REG}, x0, 1  # Should be Flushed", "jal_jump:", c_line]
...
def main():
    ...
    generate_2_instr_hazards()
    print("\nAll 2-instruction hazard tests generated.")
